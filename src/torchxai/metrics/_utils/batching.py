...
def _divide_and_aggregate_metrics_n_perturbations_per_feature(
    ...
    """
    if max_examples_per_batch is not None and (
        max_examples_per_batch // n_perturbations_per_feature < 1
        or max_examples_per_batch // n_perturbations_per_feature > n_features
    ):
        warnings.warn(
            (
                "`max_examples_per_batch` must be at least equal to the"
                ...
                "the original example and doesn't exceed the number of "
                "expanded n_features."
            ).format(max_examples_per_batch, n_perturbations_per_feature)
        )
    ...
    max_inps_per_batch = (
        ...
        if max_examples_per_batch is None
            ...
            max(max_examples_per_batch // n_perturbations_per_feature, 1), n_features
        )
    )
...
def _divide_and_aggregate_metrics_n_features(
    ...
    max_inps_per_batch = (
        n_features
        if max_examples_per_batch is None
        else min(max_examples_per_batch, n_features)
    )
