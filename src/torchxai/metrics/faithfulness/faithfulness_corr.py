...
from torchxai.metrics._utils.common import (
    ...
    _generate_random_perturbation_masks_v2,
...
def faithfulness_corr(
    ...
    r"""
        ...
        Args:
            ...
            perturb_func (Callable):
                    ...
                    `faithfulness_corr_perturb_func_decorator` decorator such as:

                    >>> from captum.metrics import faithfulness_corr_perturb_func_decorator

                    >>> @faithfulness_corr_perturb_func_decorator(<multipy_by_inputs flag>)
                    >>> def my_perturb_func(inputs):
                    >>>   <MY-LOGIC-HERE>
                    ...
                    flag is True we compute it by dividingimport sys
    sys.path.append("../src/")
                       ...
                       perturbed inputs in case `faithfulness_corr_perturb_func_decorator`
            ...
            attributions (Tensor or tuple[Tensor, ...]):
                    ...
                    If we want to compute the faithfulness_corr of global attributions we
                    ...
                    `faithfulness_corr_perturb_func_decorator` function decorator is a helper
                    function that computes perturbations under the hood if perturbed
                    inputs are provided.

                    For more details about how to use `faithfulness_corr_perturb_func_decorator`,
            ...
            feature_masks (Tensor or tuple[Tensor, ...]):
                    Feature masks that corresponds to the attributions. If feature masks are provided,
                    the faithfulness correlation is computed by perturbing groups of features instead of individual
                    features. The feature masks should have the same shape as the attributions but not necessarily
                    as the input. The feature masks should be constructed in a way that each feature in the input tensor
                    is assigned to a unique group.
            ...
            normalize (bool, optional): Normalize the dot product of the input
                    perturbation and the attribution so the faithfulness_corr value is invariant
        ...
        Returns:
            ...
            faithfulness_corr (Tensor): A tensor of scalar faithfulness_corr scores per
        ...
        Examples::
            ...
            >>> # Computes faithfulness_corr score for saliency maps
            >>> faithfulness_score = faithfulness_corr(net, perturb_fn, input, attribution)
    """
    ...
    with torch.no_grad():
        ...
        global_perturbation_masks = _generate_random_perturbation_masks_v2(
            n_perturb_samples,
            n_grouped_features,
