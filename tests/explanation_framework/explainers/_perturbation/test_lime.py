...
class LimeExplainerTest(ExplainersTestBase):
    ...
    def test_basic_single(self) -> None:
        self.basic_single_test_setup(
            ...
            expected_explanation=(torch.tensor([1.4187]), torch.tensor([-0.5716])),
            delta=1e-4,
        )
    ...
    def test_basic_batched(self) -> None:
        self.basic_batched_test_setup(
            ...
            expected_explanation=(
                torch.tensor([1.4187, 1.4702, 1.4610]),
                torch.tensor([-0.5716, -0.4955, -0.6167]),
            ),
            delta=1e-4,
        )
